import {socialHandler} from './src/social.mjs';
...
http.createServer(async(req,res)=>{try{const chunks=[];let size=0;for await(const chunk of req){size+=chunk.length;if(size>27*1024*1024){res.writeHead(413);res.end();return;}chunks.push(chunk);}const origin=process.env.PUBLIC_ORIGIN||`http://127.0.0.1:${process.env.PORT||4173}`;const request=new Request(origin+req.url,{method:req.method,headers:req.headers,...(!['GET','HEAD'].includes(req.method)?{body:Buffer.concat(chunks)}:{})});const result=(new URL(request.url).pathname.startsWith('/api/social/')?await socialHandler(request):await worker.fetch(request,{DB,UPLOADS,ASSETS}));res.writeHead(result.status,Object.fromEntries(result.headers));res.end(Buffer.from(await result.arrayBuffer()));}catch(e){res.writeHead(500);res.end('Server error');console.error(e.message);}}).listen(Number(process.env.PORT)||4173,'127.0.0.1',()=>console.log(`Local: http://127.0.0.1:${process.env.PORT||4173}`));
